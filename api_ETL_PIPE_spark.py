...
import sqlite3
from pipe_model_ckdata import CSV_Quality_checker 
...
API_KEY = 'RGAPI-a8a819c6-8a56-4417-bfc5-f34935cade07'
...
def check_stability(**context):
    '''check pandas's df distribution, vif correlation'''
    ti = context['ti']
    df = ti.xcom_pull(task_ids = 'process_api_data', key = 'df')
    print(df.head())
    
    CQ = CSV_Quality_checker(df)
    vif_value = CQ.calculation_vif('QType')
...
# email send : email operator
    ...
    check_data_stability = PythonOperator(
        ...
        python_callable = check_stability,
        dag = dag
    )
...
api_check >> extract_data >> process_api_data >> check_data_stability